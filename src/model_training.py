...
experiment_name = "LAX_Departure_Delay_Regression Fixed"
mlflow.set_experiment(experiment_name)

env_run_id = os.environ.get("_MLFLOW_RUN_ID")
env_experiment_id = os.environ.get("MLFLOW_EXPERIMENT_ID")
print(f"MLflow Project's _MLFLOW_RUN_ID: {env_run_id}")
print(f"MLflow Project's MLFLOW_EXPERIMENT_ID: {env_experiment_id}")
...
def main():
    ...
    with mlflow.start_run(run_name="RidgeReg", nested=True):
    ...
    tips = pd.DataFrame()
    tips["prediction"] = pd.Series(result.ravel().astype(float))
    ...
    mlflow.start_run(run_name = "Final Model - Test Data", nested=True)
